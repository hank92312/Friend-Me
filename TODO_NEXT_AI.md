...
- **結果揭曉畫面 (Phase 4) 房號顯示與複製按鈕實機測試 (2026-05-26 待玩家實測驗證)**：
  - **待實測細節**：
    1. 進入結果揭曉（Phase 4）畫面，確認左上角是否正確顯示當前房間號碼（格式如 `房間碼: ABC123`），且按鈕與右側「離開圈圈」在直式螢幕上完美對稱。
    2. 點擊「複製」按鈕，確認是否能成功複製房間碼，並確認點選後按鈕文字是否提示「已複製」，並在 1.5 秒後恢復為「複製」。
    3. 切換為 English 語言，確認該區域是否能自動對齊顯示為 `Room Code: ABC123` 與 `Copy`。
...
- **選等級/選題階段超時，結果揭曉題目異常修正 (2026-05-27 已完成修復，待玩家實測驗證)**：
  - **問題原因**：當選題/選等級階段 (Phase 1) 超時，後端 Fly.io 會執行自動隨機選題。但因為生產環境的 Docker 映像檔並未包含 `question_bank.json` 題庫檔案，導致後端讀取失敗回退並廣播防呆文字 `(自動選題 - 題庫未載入)`，進而使客戶端在 Answering 及 Revelation (Phase 4) 階段無法正常顯示真正的題目。
  - **修復內容**：
    1. 將 `question_bank.json` 與 `question_bank_en.json` 複製一份至 `backend/data/` 中，以確保其會隨 `backend` 目錄一起打包進生產環境 Docker 容器中。
    2. 修正 `backend/main.py` 中的 `load_question_bank()` 載入路徑，新增容器內的本地 `/app/data/question_bank.json` 支援。
    3. 本地執行測試確認題庫能被順利載入且自動隨機選題回傳正常字串。
- **結果揭曉房號顯示與句號折行修正等優化 (2026-05-26 已完成修復，待玩家實測驗證)**：
  - **修復內容**：
    1. **結果揭曉畫面 (Phase 4) 左上角新增房號顯示與複製按鈕**：在結果揭曉左上角加入了絕對定位的 HBox，顯示房號（如 `房間碼: ABC123`）並提供複製按鈕，與右側的「離開圈圈」按鈕對稱，且完全不影響中間 Results 列表的自適應佈局。
    2. **遊戲說明第三頁 Step 1 句號孤行折行修正**：精簡 Step 1 的中文文字為 `房主選擇「話題深度」(LV1~LV5)。`，徹底解決了原本最後一個句號單獨掉落至下一行的排版瑕疵。
- **手機端 App 縮小至背景再切回之倒數不同步修復 (2026-05-26 已完成修復，待玩家實測驗證)**：
  ...
  - **修復內容**：
    ...
    2. **重連自動同步**：在 `_on_reconnect_status` 重連事件中接收伺服器回傳 the `remaining_seconds` 並重新校正本地截止時間，確保斷線重連後的計時器 100% 精準。
