...
## 9. 最近重要更新紀錄 (Recent Updates - 2026-05-26)

### 1. 手機端縮小至背景再切回之倒數不同步與重連優化 (2026-05-26)
- **問題分析**：在手機（APK）與網頁端，若 App 暫時縮小到背景，Godot 引擎會被系統暫停（`_process` 停止執行），導致基於 `delta` 累減的本地倒數計時器凍結。當切回 App 時，本地時間會落後於其他玩家，造成不同步。
- **修復成果**：
  1. **Unix 截止時間戳記比對**：全面改用 `Time.get_unix_time_from_system()`，改以「目標截止 Unix 時間戳記」與「當前系統時間」的差值來決定剩餘秒數。App 從背景恢復後，剩餘秒數會自動扣減過去的時間，達成完美同步。
  2. **重連自動同步**：在 `_on_reconnect_status` 重連回覆事件中，如果伺服器傳回的 `remaining_seconds > 0`，則自動校正並更新本地截止時間戳記，確保斷線重連後的計時器 100% 精準。
  3. **倒數計時超時強制推進**：若玩家（在選題階段是隊長、在配對階段或結果揭曉階段）在本地倒數歸零時未提交，App 會在本地計時歸零時主動執行強制預設操作推進，避免在伺服器超時任務稍微延遲時出現等待卡死。且結果揭曉階段設為 120 秒，其餘階段設為 60 秒。
  4. **遊戲說明修改**：將說明第一頁的第二項由「輸入一個朋友認得的暱稱」修改為更簡單清晰的「輸入你的名字(暱稱)」（中英文已同步）。

### 2. 本地/雲端開發伺服器環境動態切換 (Option 1 - 2026-05-26)
- 在 `Phase0_Lobby` 畫面右下角新增了一個 `BtnEnvToggle` 環境切換按鈕。
- 在 `_ready()` 中利用 `OS.is_debug_build()` 進行平台檢測，確保此按鈕**僅在 Debug 測試版中顯示與啟用**，在最終發行（Release）版本中會自動隱藏，保護伺服器隱私。
- 點擊按鈕可在本機測試伺服器（localhost:8000）與 Fly.io 雲端伺服器（friends-and-me.fly.dev）之間動態切換，更新 `network_manager.gd` 內部的 `BASE_URL` 與 `WS_URL`，免除反覆編譯的繁瑣。

### 3. 行動端網頁貼上與全域字體大小優化 (2026-05-25)
- **Safari 貼上支援**：因行動端網頁（特別是 iOS Safari）出於安全政策限制無法讀取剪貼簿，實作了 JavaScriptBridge 搭配 `prompt()` 備用彈窗，在剪貼簿被阻擋時彈出原生貼上框供玩家長按貼上暱稱與房號。
- **全域字體動態放大**：實作遞迴字體大小縮放，自動將網頁端（Web）字型大小增加 12 單位、手機端（APK）增加 10 單位，極大地提升了答案 Pill 按鈕與結算結果的可讀性。
- **Dialog 彈窗排版塌陷與自適應修復**：實作 `_adjust_single_dialog_card()` 動態調整 ScrollContainer 的 custom_minimum_size 高度，防範在 Godot CenterContainer 佈局下彈窗塌陷為窄線，並完成中英文換行自適應（中文為字元任意換行，英文為單字完整換行）。

### 4. UI 懸浮面板置中自適應優化 (2026-05-21)
