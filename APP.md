...
### 1. 手機端 App 縮小至背景再切回之倒數不同步修復
- **問題解決**：針對 Android/iOS 系統於 App 進入背景後掛起暫停 `_process` 導致倒數秒數「凍結」的不同步 Bug，將本地計時改為記錄「截止的 Unix 時間戳記」（`Time.get_unix_time_from_system()`），切回前台時透過差值計算剩餘時間，時間會自動扣除。
- **重連自動同步**：在 `_on_reconnect_status` 事件中獲取伺服器回傳的 `remaining_seconds` 並重新校準本地截止時間戳記，確保斷線重連後的計時器 100% 精準。

### 2. 倒數計時防卡死強制前進功能 (1 分鐘與 2 分鐘倒數)
- 在選題（SELECTION）、配對（GUESSING）及結果揭曉（REVELATION）階段，當本地計時器歸零時，用戶端將主動執行強制預設操作（隊長自動隨機選題、玩家自動提交已配對答案、結果揭曉自動準備下一輪），避免有玩家不操作或伺服器超時任務延遲時遊戲卡死。
- 確認結果揭曉倒數時間為 120 秒（2分鐘），而選題、答題、配對為 60 秒（1分鐘）。

### 3. Debug 本地/雲端環境切換按鈕 (Option 1)
- 在 Lobby 大廳右下角新增 debug 專屬的「本地/雲端伺服器切換」按鈕，允許開發時一鍵切換 local 與 cloud API 及 WebSocket 網址。
- 使用 `OS.is_debug_build()` 進行動態判斷，確保在生產釋出版（Release Build）中此按鈕會自動被隱藏且禁用。

### 4. Dialog 彈窗排版與雙語折行自適應優化
- 解決遊戲說明、設定選項、廣告警語在 `ScrollContainer` 下因 Godot 排版特性導致的卡片塌陷 Bug，改由 viewport resized 時動態計算子節點結合最小高度重新調整。
- 實作中英文雙語的動態 autowrap 斷行模式：中文使用 `AUTOWRAP_ARBITRARY` 防拉伸爆版，英文使用 `AUTOWRAP_WORD_SMART` 防單字斷裂，並全域遞迴套用到所有靜態與動態產生的 Label 與按鈕上，徹底解決排版跑版。

### 5. 貼上功能與全域字體大小優化
- **貼上按鈕**：為解決 iOS Safari 的安全沙盒限制，實作了 JavaScriptBridge 配合 Clipboard API，並加入 prompt 彈窗作備用；暱稱輸入框右側同樣新增「貼上」按鈕，並對對應節點進行了包裹。
- **全域字體加大**：實作全域遞迴字體縮放，自動將全域文字字型在網頁端增加 `12`、手機端增加 `10` 單位偏移，並移除了豆腐塊碼表亂碼，大幅提升手機端與網頁端的易用性。

---

## 10. 歷史更新紀錄 (Recent Updates - 2026-05-21)
